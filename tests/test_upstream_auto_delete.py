...
class UpstreamAutoDeleteTests(unittest.IsolatedAsyncioTestCase):
    async def test_runtime_cleanup_releases_account_without_deleting_chat_by_default(self) -> None:
        acc = SimpleNamespace(token="token-1", inflight=1)
    ...
    async def test_runtime_cleanup_passes_account_context_to_delete_chat(self) -> None:
        acc = SimpleNamespace(token="token-1", inflight=1)
        client = SimpleNamespace(
            account_pool=SimpleNamespace(release=AsyncMock()),
            delete_chat=AsyncMock(),
        )
        client.account_pool.release = unittest.mock.Mock(side_effect=lambda account: setattr(account, "inflight", max(0, getattr(account, "inflight", 0) - 1)))

        with patch.object(settings, "UPSTREAM_AUTO_DELETE_ENABLED", True):
            await cleanup_runtime_resources(client, acc, "chat-1")
            await asyncio.sleep(0)

        client.account_pool.release.assert_called_once_with(acc)
        client.delete_chat.assert_awaited_once_with("token-1", "chat-1", account=acc)
    ...
    async def test_context_cleanup_passes_account_context_to_delete_chat(self) -> None:
        class StopLoop(Exception):
            pass
    ...
    async def test_garbage_collector_passes_account_context_to_chat_operations(self) -> None:
        class StopLoop(Exception):
            pass
        ...
        client.list_chats.assert_awaited_once_with("token-1", limit=50, account=acc)
        client.delete_chat.assert_awaited_once_with("token-1", "chat-1", account=acc)
